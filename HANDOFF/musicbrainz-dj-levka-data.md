...
## Обложки — ✅ все 10 загружены (31.08.2026)

Ikigai · Flowers · Soundstates · BERSERK · Welcome · Game Over · Deep Sleep · At the Jazz Club ·
Bubblegum · Glitch — у каждого одна обложка с типом Front, проверено через `/ws/2/release/<mbid>`.

**Две ловушки, на которые я наступил:**
1. Загрузка идёт асинхронно после нажатия «Enter edit». Если сразу уйти на следующий релиз, она
   обрывается — так пропали пять из первого захода. Нужно дождаться редиректа на `/cover-art`.
2. Из-за этого же на Game Over получился дубль (первая попытка всё-таки дошла). Лишнюю картинку
   удалил через `remove-cover-art` с объяснением в примечании.

Проверять результат лучше с паузой ~2 секунды между запросами: при частых обращениях API отдаёт
неполный ответ без поля `count`, и кажется, будто обложки нет.
