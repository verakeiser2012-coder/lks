...
// lang — версия сайта ('ru' | 'en'): сеть с audience 'all' видна везде,
// 'ru'/'en' — только в своей версии.
function socialLinksGroup(name, opts = {}) {
  ...
  order[first] = -1;
  const items = db
    .prepare("SELECT key, label, url, category FROM social_networks WHERE url <> '' AND key <> 'news' AND audience IN ('all', ?) ORDER BY label")
    .all(lang)
